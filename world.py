...
class StartTile(MapTile):
    def intro_text(self):
        ...
        Also, you were bitten by a WereBear....  you are a bear.
        """
    ...
    def __init__(self,x,y):
        ...
        if r < 0.50:
            ...
            self.alive_text = "You hear a squeaking noise growing louder"\
                "...suddently you are lost in a swarm of bats!"
...
class TraderTile(MapTile):
    ...
    def check_if_trade(self, player):
        while True:
            print("Would you like like to (B)uy, (S)ell, or (Q)uit?")
            user_input = input()
            if user_input in ['Q', 'q']:
                return
            elif user_input in ['B', 'b']:
                print("Here is whats available to buy: ")
                self.trade(buyer=player, seller=self.trader)
            elif user_input in ['S', 's']:
                print("Here is whats available to sell: ")
...
def parse_world_dsl():
    ...
    for x, dsl_row in enumerate(dsl_lines):
        row =[]
        dsl_cells = dsl_row.split("|")
        dsl_cells = [c for c in dsl_cells if c]
        for y, dsl_cell in enumerate(dsl_cells):
